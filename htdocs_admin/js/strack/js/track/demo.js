...
var Test_ak = 'KrnrrmGcxmZVGB4YcxHywmwjC79NMVqm';
var ServiceId = '129616';
    ...
    return {
        ...
        initEvents: function () {
            ...
            for(var i=0;i<data_info.length;i++){
                var icon = new BMap.Icon(data_info[i][3], new BMap.Size(23,23));
            ...
            //计算预估距离
            var points = $('#linepoints').val();
            points = eval(points);
            var p1 = new BMap.Point(points[0][0],points[0][1]);
            var p2 = new BMap.Point(points[points.length-1][0],points[points.length-1][1]);
            var searchComplete = function (results){
                ...
                output = '预估距离:';
                distance = plan.getDistance(false)/1000;
                distance = distance.toFixed(1);
                output += distance + 'Km'; //获取距离
            ...
            for (var i=0; i < points.length; i++)
            {
                ...
                {
                    mapPoints[i-1] = new BMap.Point(points[i][0],points[i][1])
                }
            }

            map.addControl(new BMap.NavigationControl({anchor:BMAP_ANCHOR_TOP_RIGHT}));
            map.addControl(new BMap.ScaleControl({anchor:BMAP_ANCHOR_BOTTOM_RIGHT}));
            map2 = new BMap.Map("mapContainer2");
            var transit = new BMap.DrivingRoute(map2, {renderOptions: {map: map2},
                onSearchComplete: searchComplete,
                });
            transit.search(p1, p2,{waypoints:mapPoints});
