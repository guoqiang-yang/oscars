 ...
 * @todo 很多逻辑写的太深了，待重构！
 * 
 * @notice 文件的逻辑同：warehouse/ajax/refund_stockin.php 
 *         原因：权限系统，分离权限时处理的，需要重构
 */
...
class App extends App_Admin_Ajax
{
    ...
    {
        if ( (empty($this->id) && empty($this->srid)) || !in_array($this->optype, array(self::OP_TYPE_CONFIRM)))
        {
    ...
    {
        parent::checkAuth('/warehouse/ajax/confirm_refund_stockin');
    }
    ...
    protected function main()
    {
        ...
        if (!empty($this->id)){
            ...
            $this->wid = $this->stockInInfo['wid'];
        }
        else
        {
            $wsir = new Warehouse_Stock_In_Refund();
            $this->refund = $wsir->get($this->srid);
            $this->wid = $this->refund['wid'];
        }
        
        //锁库判断
        $lockedRet = Conf_Warehouse::isLockedWarehouse($this->wid);
        if ($lockedRet['st'])
        {
...
//            }
    ...
    private function _confirmRefundStockin()
    {
        ...
        {
            $wsip = new Warehouse_Stock_In_Product;
            $products = $wsip->getProductsOfStockIn($this->id, $this->srid);
            $sids = array_unique(Tool_Array::getFields($products, 'sid'));

            // 获取货位库存
            $wl = new Warehouse_Location();
            $stockOfLoc = $wl->getLocationsBySids($sids, $this->wid, 'actual');
            ...
            $this->stockInInfo = array(
                'oid' => 0,
                'id' => 0,
                'wid' => $this->wid,
                'sid' => $this->refund['supplier_id'],
            );
        }
        else
        {
...
//        $this->_updateFinance();
        ...
        // Cost-FIFO出队
        $refreshCostDatas = Shop_Cost_Api::getCostsWithSkuAndNums($this->wid, $this->waitRefundProduct);
        $billInfo = array('out_id'=>$this->srid, 'out_type'=>Conf_Warehouse::STOCK_HISTORY_STOCKIN_REFUND);
        foreach($refreshCostDatas as $_sid => $fifoCosts)
        {
            if (empty($fifoCosts['_cost_fifo'])) continue;

            Shop_Cost_Api::dequeue4FifoCost($_sid, $this->wid, $billInfo, $fifoCosts['_cost_fifo']);
        }

        if (empty($this->id))
        {
            $objId = $this->srid;
            $objType = Conf_Admin_Log::OBJTYPE_STOCK_IN_REFUND;
            $actionType = 1;
        }
        else
        {
            $objId = $this->stockInInfo['oid'];
            $objType = Conf_Admin_Log::OBJTYPE_IN_ORDER;
            $actionType = 7;
        }
        ...
        $info = array(
            ...
            'params' => json_encode(array('srid' => $this->srid, 'id' => $this->stockInInfo['id'])),
            'wid' => $this->stockInInfo['wid'],
        );
