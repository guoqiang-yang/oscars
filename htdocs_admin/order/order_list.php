...
//        $this->orderList4Sales = Tool_Input::clean('r', 'order_list_4_sale', TYPE_UINT);
        ...
        $this->searchConf = array(
            ...
            'delivery_date_type' => Tool_Input::clean('r', 'delivery_date_type', TYPE_STR),
        ...
        $this->deliveryDateType = Order_Helper::getOrderDateList();
    }

    protected function checkPara()
    {
        if(empty($this->searchConf['delivery_date_type']))
        {
            $this->searchConf['delivery_date_type'] = '3_month';
            $this->searchConf['from_date'] = $this->deliveryDateType['3_month']['delivery_date_begin'];
            $this->searchConf['end_date'] = $this->deliveryDateType['3_month']['delivery_date_end'];
        }
    }

    private function _setSearchConfForSaler()
    {
        if (Admin_Role_Api::hasRole($this->_user, Conf_Admin::ROLE_CS_NEW)&& $this->searchConf['saler_suid']==0)
        {
            return;
        }
        ...
        if (Admin_Role_Api::hasRole($this->_user, Conf_Admin::ROLE_CITY_ADMIN_NEW) && $this->searchConf['saler_suid']==0)
        {
            return;
        }
        ...
        //临时处理，后面改进
        if ($this->_uid == 1426 && (empty($this->searchConf['saler_suid'])||$this->searchConf['saler_suid'] = $this->_uid))
        {
            unset($this->searchConf['saler_suid']);
            $this->searchConf['city_id'] = Conf_City::BEIJING;
        }
    }
...
//        }
    ...
    protected function outputBody()
    {
        ...
        $app = '/order/order_list.php?' . $queryStr;
        ...
        $this->smarty->assign('delivery_date_type', $this->deliveryDateType);
