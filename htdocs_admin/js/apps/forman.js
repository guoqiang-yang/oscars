(function () {
    ...
    function _onSave(ev) {
        var para = {
            ...
            status: $('#status').val(),
            intro: $("#intro").val(),
            work_community: $('#work_community').val()
        };
        if (K.isEmpty(para.name)) {
            alert('姓名不能为空');
            return false;
        }
        if (K.isEmpty(para.logo)) {
            alert('请上传头像');
            return false;
        }
        if (parseInt(para.cid) == 0) {
            alert('关联cid不能为空');
            return false;
        }
        if (parseInt(para.work_age) == 0) {
            alert('工龄不能为空');
            return false;
        }
        if (K.isEmpty(para.birthplace)) {
            alert('籍贯不能为空');
            return false;
        }

        $(this).attr('disabled', true);
        K.post('/activity/ajax/save_forman.php', para, _onSaveSucc);
    }

    function _onSaveSucc(data) {
        alert('保存成功');
        window.location.href = '/activity/forman_list.php';
    }
        ...
        this.uploader = new qq.FileUploaderBasic({
            ...
            onComplete: function(id, fileName, responseJSON) {
                ...
                if(responseJSON.payload) {
                    ...
                    $('input[name=logo]').val(data.pictag);
