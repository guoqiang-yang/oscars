...
class App extends App_Admin_Ajax
{
    ...
    protected function getPara()
    {
        ...
        $this->info = array(
            ...
            'imgurl' => Tool_Input::clean('r', 'imgurl', TYPE_STR),
        );
    }
    ...
    protected function main()
    {
        ...
        {
            $this->info['online'] = 1;
            $this->id = Activity_Shortcut_Api::add($this->info);
        }
        else
        {
            if (!empty($this->method))
            {
                $item = Activity_Shortcut_Api::getOne($this->sid);
                ...
                $data = Activity_Shortcut_Api::getList($conf, 0, 1000);
                $num = 0;
                ...
                {
                    case 'up':
                        $this->id = Activity_Shortcut_Api::update($this->sid, array('online' => 0));
                        break;
                    case 'down':
                        $this->id = Activity_Shortcut_Api::update($this->sid, array('online' => 1));
                        break;
                }
            }
            if (!empty($this->sort))
            {
                $this->id = Activity_Shortcut_Api::update($this->sid, array('sort' => $this->sort));
            }
            if (empty($this->method) && empty($this->sort))
            {
                $this->id = Activity_Shortcut_Api::update($this->sid, $this->info);
            }
        }
